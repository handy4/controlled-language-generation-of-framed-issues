...
parser.add_argument("filename", help="input filename (without warc.gz!)",
                    type=str)
...
i = 0       
with open("pipeline_input/" + filename + ".warc", 'rb') as stream:
    for record in ArchiveIterator(stream):
        i += 1
        if (i % 1000 == 0):
            print("Extraction Progress:", i)
        if record.rec_type == 'response':
            html = record.content_stream().read()
            if len(html) == 0:
                continue
            try:
                article = justext.justext(html, justext.get_stoplist("English"))
            except ParserError:
                print("Parser Error!")
                continue
            text = ""
...
num_sentences_accepted = len(final_df)

final_df.to_csv("pipeline_output/" + filename + ".csv", sep="\t", index = None)
print(i)
print(num_docs_accepted)
print(num_sentences)
print(num_sentences_accepted)
