...
def train(model, dataset, optimizer, criterion, epoch, args, data_start_index):
    ...
    for batch_num, batch in enumerate(tqdm(loader, total=len(loader))):
        ...
        elif args.task in ['ftopic', 'fframe']:
            use_indices = classification_targets != -1
            loss = criterion(scores[use_indices], classification_targets[use_indices])
        else: # topic, rhyme
    ...
    with torch.no_grad():
        for batch_num, batch in enumerate(tqdm(loader, total=len(loader))):
            ...
            if args.task == 'formality': # we're learning for all positions at once. scores are batch x seq
                ...
                use_indices = classification_targets != -1
                loss = criterion(scores[use_indices], classification_targets.float()[use_indices])
            else: # topic, rhyme
                loss = criterion(scores.flatten(), labels.flatten().float())
            loss_meter.update(loss.detach(), len(labels))
            if batch_num % args.train_print_freq == 0:
...
if __name__=='__main__':
    ...
    # DATA
    parser.add_argument('--task', type=str, required=True, choices=['iambic', 'rhyme', 'newline', 'topic', 'formality', 'ftopic', 'fframe'])
