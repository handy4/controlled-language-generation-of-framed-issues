...
def collate(batch):
    ...
    future_words = [b[2] for b in batch]
    fw_max_length = ATTRIBUTE_MAX_LENGTH
    for i in range(len(future_words)):
        if len(future_words[i]) < fw_max_length :
            future_words[i] = torch.cat([future_words[i], torch.zeros(fw_max_length - len(future_words[i])).long()], dim=0) # actually 0 is fine as pad since it's masked out
    future_words = torch.stack(future_words, dim=0)
...
class Dataset:
    def __init__(self, args):
        ...
        self.ftopic = args.task == 'ftopic'
        self.fframe = args.task == 'fframe'
        ...
        if self.formality:
            ...
            with open(os.path.join(args.data_dir, "fudge-input.obj"), 'rb') as rf:
                data_dict = pickle.load(rf)
                print(len(data_dict["input"]))
            for i in range(len(data_dict["input"])):
                if (i % 5000 == 0):
                    print(i)
                sentences.append((data_dict["input"][i], data_dict["topic"][i], data_dict["label"][i]))
            self.vocab = defaultdict(lambda: 0, data_dict["vocab"])
            random.shuffle(sentences)
            self.splits = {}
            self.splits['val'] = sentences[:FTOPIC_VAL_SIZE]
            self.splits['test'] = sentences[FTOPIC_VAL_SIZE:2*FTOPIC_VAL_SIZE]
            self.splits['train'] = sentences[2*FTOPIC_VAL_SIZE:]

        elif self.fframe:
            with open(os.path.join(args.data_dir, "fudge-input-frames.obj"), 'rb') as rf:
                data_dict = pickle.load(rf)
                print(len(data_dict["input"]))
            for i in range(len(data_dict["input"])):
                if (i % 5000 == 0):
                    print(i)
                sentences.append((data_dict["input"][i], data_dict["frame"][i], data_dict["label"][i]))
            self.vocab = defaultdict(lambda: 0, data_dict["vocab"])
            random.shuffle(sentences)
            self.splits = {}
            self.splits['val'] = sentences[:FTOPIC_VAL_SIZE]
            ...
            for root, _, filenames in os.walk(args.data_dir):
                for fname in filenames:
                    with open(os.path.join(root, fname), 'r', encoding="utf-8") as rf:
                        for line in rf:
        ...
        if args.dataset_info is not None:
            ...
            if args.glove_file is None:
                ...
                glove_embeddings = {}
                with open(args.glove_file, 'r', encoding="utf-8") as rf:
                    for i, line in enumerate(rf):
        ...
        if self.rhyme:
            if args.rhyme_info is not None:
                ...
                with open(args.rhyme_info, 'rb', encoding="utf-8") as rf:
                ...
                if len(sentence) > min_sentence_length: # set to 3. well, everything in data is > 3 for the bag of words task
                    ...
                    if not failed and num_words_in_input < len(original_sentence):
                        ...
                        if not failed and future_word in self.parent.word2index.keys():
                            ...
                            valid = not failed
            elif self.parent.ftopic:
                failed = False
                future_word_num_syllables, rhyme_group_index, syllables_to_go, word_log_prob = -1, -1, -1, -1
                raw_input = self.data[self.pos]                          
                inp = raw_input[0]
                length = len(inp)    
                classification_label = [raw_input[2] for _ in range(length)]                                
                future_word = raw_input[1]
                pad_id = self.parent.gpt_pad_id
                example = (inp, length, future_word, word_log_prob, pad_id, classification_label, syllables_to_go, future_word_num_syllables, rhyme_group_index)
                valid = not failed
            elif self.parent.fframe:
                failed = False
                future_word_num_syllables, rhyme_group_index, syllables_to_go, word_log_prob = -1, -1, -1, -1
                raw_input = self.data[self.pos]                          
                inp = raw_input[0]
